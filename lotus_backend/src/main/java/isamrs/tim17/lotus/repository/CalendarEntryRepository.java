...
public interface CalendarEntryRepository extends JpaRepository<CalendarEntry, Long> {
	...
	@Query("SELECT c FROM CalendarEntry c where (c.appointment.room = :room OR c.operation.room = :room) AND (c.startDate BETWEEN :startDate AND :endDate) order by c.startDate")
